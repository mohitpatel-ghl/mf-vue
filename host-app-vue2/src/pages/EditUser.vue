<template>
    <div class="edit-user-container p-6 border-4 border-double border-green-400 rounded-xl bg-green-50 shadow-lg">
      ...
      <div ref="remoteAppContainer" class="mt-4 p-4 border-2 border-dashed border-blue-400 rounded-lg bg-blue-50">
        <p v-if="loading" class="text-center text-blue-700 text-xl animate-pulse">Loading remote Vue 3 Edit User component...</p>
        <p v-if="error" class="text-center text-red-600 text-xl">Error loading edit user vue 3 remote: {{ error.message }}</p>
      </div>
    </div>
  </template>
