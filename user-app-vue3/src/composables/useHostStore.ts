...
export function useHostStore() {
  ...
  if (!storeAdapter) {
    ...
    return {
      ...
      updateUser: async (id: number, userData: Partial<Omit<User, 'id'>>) => {},
  ...
  const addUser = (userData: Omit<User, 'id'>) => storeAdapter.addUser(userData);
  const updateUser = (id: number, userData: Partial<Omit<User, 'id'>>) => storeAdapter.updateUser(id, userData);
